...
export default function ScholarshipManage() {
  ...
  const calculateStats = (data) => {
    ...
    const recruiting = data.filter(
      (s) => s.recruitmentStatus === "OPEN"
    ).length;
  ...
  const transformScholarshipData = (scholarship) => ({
    ...
    tag:
      scholarship.recruitmentStatus === "OPEN"
        ? "모집중"
        : scholarship.recruitmentStatus === "CLOSED"
        ? "모집완료"
        : "모집예정",
    ...
    method:
      scholarship.evaluationMethod === "DOCUMENT_INTERVIEW"
        ? "서류 + 면접"
        : "서류 심사",
          ...
          <FilterBar
            ...
            filters={[
              ...
              { label: "전체", type: "radio", name: "status", value: true },
              {
                label: "카테고리",
                type: "select",
                options: ["전체", "성적", "생활", "활동", "기타"],
              },
              {
                label: "상태",
                type: "select",
                options: ["전체", "모집중", "모집예정", "모집완료"],
              },
            ]}
          />
